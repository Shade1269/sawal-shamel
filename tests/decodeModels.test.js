...
async function setupTemporaryWorkspace(manifest) {
  ...
  const scriptSource = await readFile(resolve(repoRoot, 'scripts/decode_models.mjs'), 'utf8');
  ...
  await writeFile(resolve(tmpRoot, 'scripts/decode_models.mjs'), scriptSource);

  const tempManifest = {
    ...manifest,
    outputsDir: 'public/models',
  };
  await writeFile(
...
test('decode_models writes all manifest files to the output directory', async () => {
  ...
  try {
    await execFileAsync('node', ['scripts/decode_models.mjs'], { cwd: tmpRoot });
