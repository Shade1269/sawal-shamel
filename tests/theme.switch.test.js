...
test('applyThemeToDocument updates root data-theme and token values', () => {
  ...
  assert.equal(
    globalThis.getComputedStyle(root).getPropertyValue('--surface').trim(),
    themeTokens.default['--surface']
  );
  ...
  assert.equal(
    globalThis.getComputedStyle(root).getPropertyValue('--primary').trim(),
    themeTokens.luxury['--primary']
  );
  ...
  assert.equal(
    globalThis.getComputedStyle(root).getPropertyValue('--focus-ring').trim(),
    themeTokens.damascus['--focus-ring']
  );
});
