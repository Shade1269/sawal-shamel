...
-- إنشاء مستخدمين تجريبيين (50 عميل)
DO $$
DECLARE
    i INTEGER;
...
BEGIN
    -- إنشاء ملفات المستخدمين التجريبية
    FOR i IN 1..50 LOOP
    ...
    FOR i IN 1..15 LOOP
        ...
        INSERT INTO public.shops (
            ...
            '/lovable-uploads/60b94ef9-9e25-4acc-ab85-a92fc7810b69.png',
            true,
        ...
        -- إضافة إعدادات متجر مفصلة
        INSERT INTO public.shop_settings_extended (
            shop_id,
            vat_enabled,
            vat_rate,
            currency,
            ...
            jsonb_build_object(
                'accepted_methods', ARRAY['credit_card', 'bank_transfer', 'mada'],
                'auto_capture', true
            ),
            jsonb_build_object(
                'meta_title', 'متجر ' || i || ' - أفضل المنتجات',
                'meta_description', 'اكتشف مجموعة رائعة من المنتجات عالية الجودة'
            )
        );
    END LOOP;
    ...
    FOR i IN 1..200 LOOP
        ...
        INSERT INTO public.products (
            ...
            ARRAY['/lovable-uploads/0e23a745-b356-4596-95c0-39b0050acbd1.png', '/lovable-uploads/60b94ef9-9e25-4acc-ab85-a92fc7810b69.png'],
            true,
            (100 + (random() * 900))::INTEGER,
            jsonb_build_object(
                'length', (10 + random() * 40)::NUMERIC(5,2),
                'width', (10 + random() * 30)::NUMERIC(5,2),
                'height', (5 + random() * 20)::NUMERIC(5,2)
            ),
        ...
        IF i % 3 = 0 THEN
            INSERT INTO public.product_reviews (
                ...
                (3 + (random() * 2))::INTEGER,
                CASE (i % 4)
                    WHEN 0 THEN 'منتج رائع جداً، جودة ممتازة وتوصيل سريع. أنصح بشرائه بشدة!'
                    WHEN 1 THEN 'ممتاز! يستحق السعر ولم أندم على شرائه. خدمة عملاء متميزة.'
                    WHEN 2 THEN 'منتج جيد جداً، مطابق للوصف والصور. وصل في الوقت المحدد.'
                    ELSE 'راضي عن المنتج، جودة جيدة وسعر معقول. سأتسوق من هنا مرة أخرى.'
                END,
                true,
    ...
    FOR i IN 1..150 LOOP
        ...
        INSERT INTO public.ecommerce_orders (
            ...
            0, -- سيتم حساب الضريبة في trigger
            CASE WHEN i % 5 = 0 THEN (random() * 100)::NUMERIC(10,2) ELSE 0 END,
            0, -- سيتم حساب المجموع في trigger
            CASE i % 3
    ...
    SELECT 
        ...
        eoi.commission_rate,
        (eoi.total_price_sar * eoi.commission_rate / 100) as amount_sar,
        CASE 
        ...
        END::text as status,
        eoi.created_at,
        CASE WHEN random() < 0.7 THEN eoi.created_at + interval '7 days' ELSE NULL END
    FROM ecommerce_order_items eoi
    JOIN ecommerce_orders eo ON eo.id = eoi.order_id
    JOIN profiles p ON p.id = eo.user_id AND p.role = 'affiliate'
    WHERE random() < 0.3; -- 30% من الطلبات لها عمولات
