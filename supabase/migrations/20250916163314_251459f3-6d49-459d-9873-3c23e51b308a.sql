-- إنشاء دالة لإنشاء المتغيرات من المنتجات الموجودة
CREATE OR REPLACE FUNCTION create_variants_from_existing_products()
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
                ...
                LOOP
                    ...
                    IF NOT EXISTS (
                        ...
                        INSERT INTO product_variants (
                            ...
                            product_record.id,
                            variant_name_text,
                            'P-' || LEFT(product_record.id::text, 8) || '-' || color_val || '-' || size_val,
                            color_val,
                            size_val,
    ...
    RETURN 'تم إنشاء ' || variants_created || ' متغير بنجاح';
END;
$$;

-- تشغيل الدالة
SELECT create_variants_from_existing_products() as result;