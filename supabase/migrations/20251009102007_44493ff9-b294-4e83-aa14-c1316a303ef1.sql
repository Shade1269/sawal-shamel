-- ========================================
-- المرحلة 3 - الجزء 2: Catalog, Inventory, CRM, Themes
-- ========================================
...
-- ============================================
-- 4) Affiliate/Store Settings & Coupons
-- ============================================
...
DO $$ BEGIN
  IF EXISTS (
    ...
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_affiliate_settings_store'
  ) THEN
    CREATE INDEX IF NOT EXISTS idx_affiliate_settings_store_id ON public.affiliate_store_settings(store_id);
    
    ALTER TABLE public.affiliate_store_settings
      ADD CONSTRAINT fk_affiliate_settings_store
...
DO $$ BEGIN
  IF EXISTS (
    ...
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_store_settings_shop'
  ) THEN
    CREATE INDEX IF NOT EXISTS idx_store_settings_shop_id ON public.store_settings(shop_id);
...
COMMENT ON CONSTRAINT fk_variants_product ON public.product_variants IS 'متغير المنتج → المنتج الأساسي';
COMMENT ON CONSTRAINT fk_leads_store ON public.leads IS 'عميل محتمل → متجر أفلييت';
COMMENT ON CONSTRAINT fk_theme_custom_store ON public.visual_theme_customizations IS 'تخصيص ثيم → متجر أفلييت';