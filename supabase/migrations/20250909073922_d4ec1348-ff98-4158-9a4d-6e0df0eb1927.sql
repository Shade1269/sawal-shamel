-- Create cron job for token refresh every 15 minutes
...
-- Create cron job for product sync every 15 minutes
SELECT cron.schedule(
  ...
  select
    net.http_post(
        ...
        headers:='{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVld3VpaW9wa2N0ZHRhZXhtdHh1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTYzMjE2ODUsImV4cCI6MjA3MTg5NzY4NX0._q03bmVxGQhCczoBaOHM6mDefuFA"}'::jsonb,
        body:='{}'::jsonb
    ) as request_id;
  $$
);