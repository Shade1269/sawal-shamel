...
CREATE INDEX idx_user_levels_level ON public.user_levels(current_level);
CREATE INDEX idx_user_levels_points ON public.user_levels(total_points DESC);
...
-- فهارس للأداء
CREATE INDEX idx_alliances_leader_id ON public.alliances(leader_id);
CREATE INDEX idx_alliances_status ON public.alliances(status);
CREATE INDEX idx_alliances_points ON public.alliances(total_points DESC);
CREATE INDEX idx_alliances_name ON public.alliances(name);
...
-- فهارس للأداء
CREATE INDEX idx_alliance_members_alliance_id ON public.alliance_members(alliance_id);
CREATE INDEX idx_alliance_members_user_id ON public.alliance_members(user_id);
CREATE INDEX idx_alliance_members_contribution ON public.alliance_members(contribution_points DESC);
...
CREATE INDEX idx_weekly_challenges_status ON public.weekly_challenges(status);
CREATE INDEX idx_weekly_challenges_type ON public.weekly_challenges(challenge_type);
...
CREATE INDEX idx_challenge_participations_alliance ON public.challenge_participations(alliance_id);
CREATE INDEX idx_challenge_participations_rank ON public.challenge_participations(rank);

-- جدول سيطرة القلعة
CREATE TABLE public.castle_control (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  alliance_id UUID NOT NULL REFERENCES public.alliances(id) ON DELETE CASCADE,
...
-- فهارس للأداء
CREATE INDEX idx_castle_control_current ON public.castle_control(is_current, controlled_until);
CREATE INDEX idx_castle_control_alliance ON public.castle_control(alliance_id);
CREATE INDEX idx_castle_control_week ON public.castle_control(week_number, year_number);
...
-- فهارس للأداء
CREATE INDEX idx_weekly_leaderboard_week ON public.weekly_leaderboard(week_number, year_number);
CREATE INDEX idx_weekly_leaderboard_rank ON public.weekly_leaderboard(week_number, year_number, rank);
CREATE INDEX idx_weekly_leaderboard_user ON public.weekly_leaderboard(user_id);
...
-- فهارس للأداء
CREATE INDEX idx_monthly_leaderboard_month ON public.monthly_leaderboard(month_number, year_number);
CREATE INDEX idx_monthly_leaderboard_rank ON public.monthly_leaderboard(month_number, year_number, rank);
CREATE INDEX idx_monthly_leaderboard_user ON public.monthly_leaderboard(user_id);

-- جدول ترتيب التحالفات الأسبوعي
CREATE TABLE public.alliance_weekly_leaderboard (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  alliance_id UUID NOT NULL REFERENCES public.alliances(id) ON DELETE CASCADE,
  week_number INTEGER NOT NULL,
  year_number INTEGER NOT NULL,
...
-- فهارس للأداء
CREATE INDEX idx_alliance_weekly_leaderboard_week ON public.alliance_weekly_leaderboard(week_number, year_number);
CREATE INDEX idx_alliance_weekly_leaderboard_rank ON public.alliance_weekly_leaderboard(week_number, year_number, rank);
CREATE INDEX idx_alliance_weekly_leaderboard_alliance ON public.alliance_weekly_leaderboard(alliance_id);
...
CREATE INDEX idx_user_themes_active ON public.user_themes(user_id, is_active);

-- جدول التقارير والمخالفات
CREATE TABLE public.alliance_reports (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- فهارس للأداء
CREATE INDEX idx_alliance_reports_status ON public.alliance_reports(status);
CREATE INDEX idx_alliance_reports_alliance ON public.alliance_reports(reported_alliance_id);

-- إنشاء دالة لتحديث المستوى
...
-- إنشاء دالة لحساب ترتيب الأسبوع
