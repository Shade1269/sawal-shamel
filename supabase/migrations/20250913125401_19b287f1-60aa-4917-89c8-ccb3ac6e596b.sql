-- إضافة بيانات تجريبية باستخدام الأعمدة الموجودة

-- إضافة موردين 
INSERT INTO public.suppliers (supplier_name, supplier_number, phone, email, address, payment_terms, is_active) VALUES
('شركة التوريدات المتقدمة المحدودة', 'SUP001', '+966501234567', 'ahmed@advanced-supplies.com', 'حي العليا، الرياض 12211', 'دفع خلال 30 يوم', true),
('مؤسسة الجودة للتجارة والمقاولات', 'SUP002', '+966507654321', 'sara@quality-trade.com', 'حي الحمراء، جدة 21422', 'دفع عند الاستلام', true),
('مجموعة الشرق للإمدادات', 'SUP003', '+966551122334', 'mohamed@eastern-supplies.sa', 'حي الشاطئ، الدمام 31411', 'دفع خلال 15 يوم', true)
...
((SELECT id FROM public.warehouses WHERE code = 'WH001'), (SELECT id FROM public.suppliers WHERE supplier_number = 'SUP001'), 'جهاز لابتوب ديل انسبايرون 15', 'PROD001', 'LAPTOP-DELL-INS15', 'لابتوب ديل انسبايرون 15 بوصة، معالج i5، رام 8 جيجا، هارد SSD 256 جيجا', 'أجهزة كمبيوتر', 'Dell', true),
((SELECT id FROM public.warehouses WHERE code = 'WH001'), (SELECT id FROM public.suppliers WHERE supplier_number = 'SUP002'), 'طابعة HP ليزر جيت برو', 'PROD002', 'PRINTER-HP-LJ400', 'طابعة HP ليزر جيت برو M404n أبيض وأسود', 'طابعات', 'HP', true),
((SELECT id FROM public.warehouses WHERE code = 'WH002'), (SELECT id FROM public.suppliers WHERE supplier_number = 'SUP001'), 'شاشة سامسونج 27 بوصة', 'PROD003', 'MONITOR-SAM-27', 'شاشة سامسونج 27 بوصة Full HD LED', 'شاشات', 'Samsung', true),
((SELECT id FROM public.warehouses WHERE code = 'WH002'), (SELECT id FROM public.suppliers WHERE supplier_number = 'SUP003'), 'كيبورد لوجيتك لاسلكي', 'PROD004', 'KEYBOARD-LOG-MX', 'كيبورد لوجيتك MX Keys لاسلكي متقدم', 'ملحقات', 'Logitech', true)
